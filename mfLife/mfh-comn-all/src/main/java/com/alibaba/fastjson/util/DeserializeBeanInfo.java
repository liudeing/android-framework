...
public class DeserializeBeanInfo {
    ...
    public static DeserializeBeanInfo computeSetters(Class<?> clazz, Type type) {
        ...
        computeSettersInner(beanInfo, type, false);//被zhangyz 修改，移到外面重用
     ...
     * @param beanInfo
     * @param type
     * @param setFieldFlag -- 这是zhangyz新加的
     * @author zhangyz created on 2014-3-12
     */
    public static void computeSettersInner(DeserializeBeanInfo beanInfo, Type type, boolean setFieldFlag) {
        ...
        for (Method method : clazz.getMethods()) {
            ...
            Field field = null;
            if (setFieldFlag)
                field = getFieldRecursion(clazz, propertyName);
            else
            ...
            if (setFieldFlag)//add by zhangyz
                beanInfo.add(new FieldInfo(propertyName, method, field, clazz, type));
            else
                beanInfo.add(new FieldInfo(propertyName, method, null, clazz, type));
    ...
    public static Field getFieldRecursion(Class<?> clazz, String fieldName) {
        ...
        catch(java.lang.NoSuchFieldException noe) {
            Class<?> parent = clazz.getSuperclass();
            /*TypeVariable[] tps = parent.getTypeParameters();//泛型没办法支持
            for (TypeVariable tp : tps) {
                System.out.println(tp.getName());
            }
            Type parentType = clazz.getGenericSuperclass();
            String typeName = parentType.toString();*/
            if (parent != null) {
                return getFieldRecursion((Class<?>)parent, fieldName);
            }
            return null;
        }
        catch (Exception e) {
